...
async def login(data: LoginRequest):
    ...
    _HARDCODED = {
        ...
        "cajero": ("cajero2024", "cajero", "Cajero"),
    }
    if username_lower in _HARDCODED:
        pwd, role, nombre = _HARDCODED[username_lower]
        if data.password == pwd:
            logger.info(f"[LOGIN] Usuario {username_lower.upper()} reconocido (hardcoded)")
    ...
    role_map = {"ADMIN": "admin", "NORMAL": "user", "SALON": "salon", "COCINA": "cocina", "VISOR": "visor", "VALES": "cajero", "CAJERO": "cajero"}
