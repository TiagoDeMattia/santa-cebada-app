...
from database import ensure_schema, read_table, read_table_filtered
...
def parse_numeric_series(series):
    ...
    # Fast path: vectorized numeric parse (works for clean float strings like "6500.0")
    result = pd.to_numeric(series, errors="coerce")
    # Only apply the slow per-row parser to values that failed the fast parse
    failed_mask = result.isna() & series.notna() & (series.astype(str).str.strip() != "")
    if failed_mask.any():
        result = result.copy()
        result[failed_mask] = series[failed_mask].apply(parse_numeric_value)
    return result.fillna(0.0).astype("float64")
...
_BUSINESS_CUTOFF_HOUR = 3  # hours before this belong to the previous business day


def _business_date_series(timestamps):
    """Vectorized business-date from a tz-aware datetime Series."""
    before_cutoff = timestamps.dt.hour < _BUSINESS_CUTOFF_HOUR
    adjusted = timestamps - pd.to_timedelta(before_cutoff.astype(int), unit="D")
...
def build_period_columns(df, datetime_column):
    ...
    work_df["business_date"] = _business_date_series(timestamps)
...
def clean_pedidos(df):
    ...
    work_df["period_business_date"] = _business_date_series(work_df["period_start_dt"])
...
def retain_latest_snapshot_rows(df, snapshot_date_column="period_business_date", snapshot_time_column="scraped_at"):
    ...
    # Fast path: if no real scraped_at timestamps exist (e.g. imported historical data)
    # there's nothing to deduplicate — every row is unique by construction.
    if snapshot_time_column in df.columns and df[snapshot_time_column].isna().all():
        return df.copy()
...
def load_clean_data_bundle(start_date=None, end_date=None, location_key=None):
    ensure_schema()
    # Push date filters to SQL — avoids loading 200k+ historical rows when only a range is needed.
    # Add a 31-day buffer on both sides so period-boundary snapshots aren't missed.
    sql_start = (start_date - timedelta(days=31)).isoformat() if start_date else None
    sql_end   = (end_date   + timedelta(days=31)).isoformat() if end_date   else None

    pedidos_raw  = read_table_filtered("pedidos",    "fecha",        sql_start, sql_end, location_key)
    productos_raw = read_table_filtered("productos",  "period_start", sql_start, sql_end, location_key)
    pagos_raw    = read_table_filtered("formas_pago", "fecha",        sql_start, sql_end, location_key)
    gastos_raw   = read_table_filtered("gastos",      "fecha",        sql_start, sql_end, location_key)
...
def filter_clean_bundle(bundle, location_selection="ambas", start_date=None, end_date=None, turno=None):
    filtered = {
    ...
    if turno in ("manana", "noche"):
        label = "Mañana" if turno == "manana" else "Noche"

        df = filtered["df_pedidos_clean"]
        if not df.empty and "order_datetime" in df.columns:
            mask = df["order_datetime"].apply(assign_turno) == label
            filtered["df_pedidos_clean"] = df[mask].copy()

        df = filtered["df_pagos_clean"]
        if not df.empty and "payment_datetime" in df.columns:
            mask = df["payment_datetime"].apply(assign_turno) == label
            filtered["df_pagos_clean"] = df[mask].copy()

        df = filtered["df_productos_clean"]
        if not df.empty and "period_start_dt" in df.columns:
            mask = df["period_start_dt"].apply(assign_turno) == label
            filtered["df_productos_clean"] = df[mask].copy()
...
def build_finance_timeseries(pedidos_df, gastos_df, freq="D", _precomputed_sales=None):
    sales_df = _precomputed_sales if _precomputed_sales is not None else build_sales_timeseries(pedidos_df, freq=freq)
    if gastos_df.empty:
...
def build_data_quality_summary(bundle, customer_summary=None):
    pedidos_df = bundle["df_pedidos_clean"]
    pagos_df = bundle["df_pagos_clean"]
