...
def build_complete_statistics(location_key, start_date=None, end_date=None, turno=None):
    loc_sql = location_key if location_key not in ("ambas", "all", None) else None
    bundle = dp.load_clean_data_bundle(start_date=start_date, end_date=end_date, location_key=loc_sql)
    ...
    pedidos   = filtered_bundle["df_pedidos_clean"]
    productos = filtered_bundle["df_productos_clean"]
    gastos    = filtered_bundle["df_gastos_clean"]
    pagos     = filtered_bundle["df_pagos_clean"]

    # Pre-compute timeseries once per frequency — reused by both sales_timeseries and finance_timeseries
    ts_d = dp.build_sales_timeseries(pedidos, freq="D")
    ts_w = dp.build_sales_timeseries(pedidos, freq="W")
    ts_m = dp.build_sales_timeseries(pedidos, freq="M")

    # customer_summary is expensive — compute once, pass to data_quality
    customer_summary = dp.build_customer_summary(pedidos, pagos)
    ...
    return sanitize_json_compatible({
        ...
        "sales_timeseries_daily":   dataframe_to_records(ts_d),
        "sales_timeseries_weekly":  dataframe_to_records(ts_w),
        "sales_timeseries_monthly": dataframe_to_records(ts_m),
        "finance_timeseries_daily":   dataframe_to_records(
            dp.build_finance_timeseries(pedidos, gastos, freq="D", _precomputed_sales=ts_d)
        ),
        "finance_timeseries_weekly":  dataframe_to_records(
            dp.build_finance_timeseries(pedidos, gastos, freq="W", _precomputed_sales=ts_w)
        ),
        "finance_timeseries_monthly": dataframe_to_records(
            dp.build_finance_timeseries(pedidos, gastos, freq="M", _precomputed_sales=ts_m)
        ),
        "payment_summary":       dataframe_to_records(dp.build_payment_summary(pagos)),
        "payment_method_detail": dataframe_to_records(dp.build_payment_method_detail(pagos)),
        "product_summary":       dataframe_to_records(dp.build_product_summary(productos)),
        "rubro_summary":         dataframe_to_records(dp.build_rubro_summary(productos)),
        "top_products_by_revenue":    dataframe_to_records(dp.build_top_products(productos, metric="revenue_total",  top_n=15)),
        "top_products_by_quantity":   dataframe_to_records(dp.build_top_products(productos, metric="quantity_total", top_n=15)),
        "low_contribution_products":  dataframe_to_records(dp.build_low_contribution_products(productos, top_n=15)),
        "product_pareto":        dataframe_to_records(dp.build_product_pareto(productos)),
        "expense_summary":       dataframe_to_records(dp.build_expense_summary(gastos)),
        "customer_summary": {
        ...
        "customer_segments": dataframe_to_records(dp.build_customer_segment_summary(customer_summary["customers"])),
        "top_customers":     dataframe_to_records(customer_summary["customers"].head(25)),
        "hourly_sales":      dataframe_to_records(dp.build_hourly_sales(pedidos)),
        "weekday_hour_heatmap": heatmap_to_payload(dp.build_weekday_hour_heatmap(pedidos)),
        "data_quality": dp.build_data_quality_summary(filtered_bundle, customer_summary=customer_summary),
        "raw_counts": {
            "orders":   int(len(pedidos)),
            "payments": int(len(pagos)),
            "products": int(len(productos)),
            "expenses": int(len(gastos)),
        },
        "turno_breakdown": dp.build_turno_breakdown(pedidos),
    })
...
if __name__ == "__main__":
    result = scrape_complete_statistics_for_period("recoleta", period="today")
