...
def _inject_credentials():
    ...
    db_path = Path.home() / ".pm2" / "nucleocheck.db"
...
from database import ensure_schema, read_table, get_app_config
from scraper import (
...
_TURNO_MANANA_END_H,   _TURNO_MANANA_END_M   = 16, 0   # 16:00
_TURNO_NOCHE_END_H,    _TURNO_NOCHE_END_M    = 3,  0   # 03:00 del día siguiente

_NOCHE_START_MINUTES = _TURNO_MANANA_END_H * 60 + _TURNO_MANANA_END_M  # 960
_NOCHE_END_MINUTES   = _TURNO_NOCHE_END_H * 60 + _TURNO_NOCHE_END_M    # 180


def _get_cash_drawer_turno(auth_h: dict, now) -> Optional[dict]:
    """
    Consulta CashDrawerList/Find/ para detectar el turno activo.
    Retorna dict con turno_nombre/is_noche/is_cerrado/open_date/manana_open_date,
    o None si la API falla.
    """
    from datetime import time as _time, timedelta as _td
    try:
        r = _requests.get(
            f"{_API_BASE}/CashDrawerList/Find/",
            headers=auth_h,
            timeout=10,
        )
        r.raise_for_status()
        ...
        # Limite inferior del dia operativo: 03:00 ART
        business_day = now.date()
        if now.hour < 3:
            business_day = business_day - _td(days=1)
        boundary = datetime.combine(business_day, _time(3, 0), tzinfo=LOCAL_TIMEZONE)

        # Solo CAJA 1 - NORMAL determina el turno (ignorar cajas de vales u otras)
        today_drawers = []
        for d in drawers:
            if not d.get("IsActive"):
                continue
            name = (d.get("Name") or "").strip()
            if "CAJA 1" not in name.upper():
                continue
            open_date_str = d.get("OpenDate")
            if not open_date_str:
                continue
            try:
                # NucleoCheck devuelve OpenDate en UTC sin sufijo; convertir a ART
                from datetime import timezone as _utc
                open_dt = datetime.fromisoformat(open_date_str).replace(tzinfo=_utc.utc).astimezone(LOCAL_TIMEZONE)
            except Exception:
                continue
            if open_dt < boundary:
                continue
            today_drawers.append((open_dt, d))

        today_drawers.sort(key=lambda x: x[0])

        open_now = [(dt, d) for dt, d in today_drawers if d.get("IsCashDrawerOpen")]

        if not open_now:
            return {"turno_nombre": "Cerrado", "is_noche": False, "is_cerrado": True,
                    "open_date": None, "manana_open_date": None}

        latest_open_dt, _ = open_now[-1]
        first_dt = today_drawers[0][0] if today_drawers else latest_open_dt

        # Caja abierta >= 14:00 ART = turno noche
        is_noche = latest_open_dt.hour >= 14
        ...
        return {
            ...
            "is_noche": is_noche,
            "is_cerrado": False,
            "open_date": latest_open_dt,
            "manana_open_date": first_dt,
        }
    except Exception:
        return None


def _turno_ranges(auth_h: Optional[dict] = None):
    """
    Devuelve (turno_nombre, is_noche, turno_start, turno_end, manana_start, manana_end).
    Si auth_h se provee, intenta deteccion dinamica via CashDrawerList/Find/.
    Fallback: horarios fijos (16:00 fin mañana, 03:00 fin noche).
    """
    from datetime import time as _time, timedelta as _td
    now = datetime.now(LOCAL_TIMEZONE)

    if auth_h is not None:
        cd = _get_cash_drawer_turno(auth_h, now)
        if cd is not None:
            if cd["is_cerrado"]:
                business_day = now.date()
                if now.hour < 3:
                    business_day = business_day - _td(days=1)
                zero = datetime.combine(business_day, _time(3, 0), tzinfo=LOCAL_TIMEZONE)
                return "Cerrado", False, zero, now, zero, zero

            open_dt  = cd["open_date"]
            is_noche = cd["is_noche"]

            if is_noche:
                t_start = open_dt
                t_end   = now
                # Mañana: desde hora fija (09:00) hasta que abrio la caja de noche
                manana_start = datetime.combine(
                    open_dt.date(),
                    _time(_TURNO_MANANA_START_H, _TURNO_MANANA_START_M),
                ...
                t_start      = open_dt
                t_end        = now
                manana_start = open_dt
                manana_end   = now

            return cd["turno_nombre"], is_noche, t_start, t_end, manana_start, manana_end

    # ── Fallback: horarios fijos ──
    ...
    for _attempt in range(2):
        try:
            ...
            with _requests.Session() as session:
                ...
                turno_nombre, is_noche, t_start, t_end, manana_start, manana_end = _turno_ranges(auth_h)
            ...
            if getattr(getattr(e, "response", None), "status_code", None) == 401 and _attempt == 0:
                _token_cache.pop(location_key, None)
                continue
            return {**_EMPTY, "error": str(e)}
        except Exception as e:
            return {**_EMPTY, "error": str(e)}
    return {**_EMPTY, "error": "Token refresh fallido"}
    ...
    for location_key in ["recoleta"]:
        ...
        if live and live["ok"]:
            row = {
                ...
                "turno_desde": live.get("turno_desde"),
...
_FOOD_RUBROS = frozenset([
    "hamburguesas", "entradas", "pizzas", "pastas",
    "ensaladas", "platos", "postres", "menu ejecutivo",
])


def get_comida_turno(location_key: str = "recoleta") -> dict:
    """
    Ratio de platos de comida por pedido para el día actual.
    Usa datos del scraper local (actualizados cada ~2h).
    """
    ...
    # 00:00-03:00 Argentina = todavía pertenece al turno noche del día anterior
    business_date = (now_arg - timedelta(days=1)).date() if hour < 3 else now_arg.date()
    turno_label   = "noche" if (hour < 3 or hour >= 16) else "manana"

    try:
        bundle   = _dp.load_clean_data_bundle(
            start_date=business_date, end_date=business_date, location_key=location_key
        )
        filtered = _dp.filter_clean_bundle(
            bundle, location_selection=location_key,
            start_date=business_date, end_date=business_date
        )
        overview     = _dp.build_overview_metrics(filtered)
        productos_df = filtered["df_productos_clean"]
        ...
        platos = 0
        if not productos_df.empty and "rubro" in productos_df.columns:
            platos = int(productos_df[productos_df["rubro"].isin(_FOOD_RUBROS)]["cantidad_num"].sum())

        pedidos = int(overview.get("order_count") or 0)
        ratio   = round(platos / pedidos, 3) if pedidos > 0 else 0.0
        ...
        return {
            ...
            "pedidos":        pedidos,
            "ratio":          ratio,
            "ratio_por_10":   round(ratio * 10, 2),
            "en_objetivo":    ratio >= 0.1,
        }
...
def get_vales_turno(location_key: str = "recoleta") -> dict:
    """
    Monitor de vales en tiempo real para el turno activo.
    Total general: Stats/GetSalesPaymentMethods (incluye efectivo y cash).
    Vales: InvoiceList/Find por punto de venta:
      PoS 4 + cfg=2  → Vales 1 (Rick)
      PoS 2          → Vales 2 (Gustavo)
      PoS 5 + cfg=4  → Vales 3 (Karina)
    """
    import concurrent.futures
    from email.utils import format_datetime as _fmt_rfc
    from datetime import timezone as _tz_utc

    OBJETIVO_PCT = float(get_app_config("vales_objetivo_pct", "15.0"))
    ...
    for _attempt in range(2):
        try:
            ...
            turno_nombre, is_noche, t_start, t_end, _, _ = _turno_ranges(auth_h)

            def _rfc(dt):
                return _fmt_rfc(dt.astimezone(_tz_utc.utc), usegmt=True)

            # Total cobrado (todos los métodos de pago, incluye efectivo)
            r_total = _requests.get(
                f"{_API_BASE}/Stats/GetSalesPaymentMethods/",
                headers=auth_h,
                params={
                    "StartDate":    format_api_datetime(t_start),
                    "EndDate":      format_api_datetime(t_end),
                    "UseTimeRange": "false",
                },
                timeout=15,
            )
            r_total.raise_for_status()
            total_cobrado = float(r_total.json().get("Total") or 0)

            # Vales por punto de venta (en paralelo)
            date_from = _rfc(t_start)
            date_to   = _rfc(t_end)

            def _fetch_v(pos_number: int, cfg_id=None) -> float:
                resp = _requests.get(
                    f"{_API_BASE}/InvoiceList/Find",
                    headers=auth_h,
                    params={"DateFrom": date_from, "DateTo": date_to, "PointOfSaleNumber": pos_number},
                    timeout=30,
                )
                resp.raise_for_status()
                invoices = resp.json()
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=3) as pool:
                fut1 = pool.submit(_fetch_v, 4, 2)
                fut2 = pool.submit(_fetch_v, 2, None)
                fut3 = pool.submit(_fetch_v, 5, 4)
                v1, v2, v3 = fut1.result(), fut2.result(), fut3.result()

            total_vales = round(v1 + v2 + v3, 2)
            objetivo    = round(total_cobrado * OBJETIVO_PCT / 100, 2)
            diferencia  = round(total_vales - objetivo, 2)
            porcentaje  = round((total_vales / total_cobrado * 100) if total_cobrado > 0 else 0.0, 2)

            return {
                "turno":           turno_nombre,
                "is_noche":        is_noche,
                "desde":           t_start.isoformat(),
                "hasta":           t_end.isoformat(),
                "total_cobrado":   total_cobrado,
                "vales": {
                    "vales1": {"nombre": "Vales 1 - Rick",    "total": v1},
                    "vales2": {"nombre": "Vales 2 - Gustavo", "total": v2},
                    "vales3": {"nombre": "Vales 3 - Karina",  "total": v3},
                },
                "total_vales":      total_vales,
                "objetivo_pct":     OBJETIVO_PCT,
                "objetivo_monto":   objetivo,
                "porcentaje_vales": porcentaje,
                "diferencia":       diferencia,
                "en_objetivo":      diferencia >= 0,
            }
        except _requests.exceptions.HTTPError as e:
            if getattr(getattr(e, "response", None), "status_code", None) == 401 and _attempt == 0:
                _token_cache.pop(location_key, None)
