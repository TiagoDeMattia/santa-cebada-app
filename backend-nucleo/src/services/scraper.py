...
def prompt_location_cli():
    ...
    while True:
        ...
        if not choice:
            return get_location_config("recoleta")
        try:
...
# ── Helpers para fetch paralelo (cada uno usa su propia sesión HTTP) ────────────

def _fetch_orders(token, start_local, end_local):
    with requests.Session() as s:
        return get_order_history(s, token, start_local, end_local)

def _fetch_payments(token, start_local, end_local):
    with requests.Session() as s:
        return get_sales_payment_methods(s, token, start_local, end_local)

def _fetch_products(token, start_local, end_local, grouping):
    with requests.Session() as s:
        return get_sales_by_product_stats(s, token, start_local, end_local, date_grouping=grouping)

def _fetch_expenses(token, start_local, end_local):
    with requests.Session() as s:
        return get_expenses(s, token, start_local, end_local)
...
def fetch_live_statistics_bundle(start_local, end_local, location, product_date_grouping=0):
    ...
    # Login secuencial (obligatorio: validate_token depende de initial_token)
    ...
    # Las 4 consultas son independientes → corren en paralelo
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
        f_ord = pool.submit(_fetch_orders,   validated_token, start_local, end_local)
        f_pay = pool.submit(_fetch_payments, validated_token, start_local, end_local)
        f_pro = pool.submit(_fetch_products, validated_token, start_local, end_local, product_date_grouping)
        f_exp = pool.submit(_fetch_expenses, validated_token, start_local, end_local)

        order_endpoint,   order_history  = f_ord.result()
        payment_endpoint, payment_data   = f_pay.result()
        product_endpoint, product_stats  = f_pro.result()
        expense_endpoint, expenses       = f_exp.result()
...
def fetch_statistics_for_all_locations(start_local, end_local, location_keys=None):
    ...
    def _fetch_one(location_key):
        location = get_location_config(location_key)
        return fetch_statistics_bundle(start_local, end_local, location, db_path=db_path)

    # Sucursales en paralelo — las escrituras a SQLite serializan solas (StaticPool)
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(selected_keys)) as pool:
        futures = [pool.submit(_fetch_one, key) for key in selected_keys]
